...
class DingTalkCallback(models.Model):
    ...
    def register_call_back(self):
        ...
        try:
            ...
            result = client.callback.register_call_back(call_list, self.token, self.aes_key, self.url)
    ...
    def update_call_back(self):
        ...
        try:
            client = dt.get_client(self, dt.get_dingtalk_config(self, self.company_id))
            result = client.callback.update_call_back(call_list, self.token, self.aes_key, self.url)
        except Exception as e:
